...
let floating=!contacts.length,tilt=!floating&&(cx<contacts[0]-.5||cx>contacts.at(-1)+.5),span=contacts.some((x,i)=>i&&x-contacts[i-1]>6),overhang=!floating&&group.some(c=>c[0]+.5<contacts[0]-3||c[0]+.5>contacts.at(-1)+3);
...
function trace(l,ps,target=l.target){let states=[{y:l.startY??9,path:[]}],cart=l.kind==='road',height=cart?1.35:.85;for(let x=.125;x<=target[0]+.875;x+=.25){let next=[];for(let y of surfaces(l,ps,x)){let prev=states.find(a=>Math.abs(a.y-y)<=(cart?.18:1.001));if(!prev)continue;let clear=true;for(let dy=.12;dy<height;dy+=.2)if(solids(l,ps,x,y-dy))clear=false;if(clear)next.push({y,path:[...prev.path,[x,y]]})}if(!next.length){let last=states[0];return{ok:false,path:last.path,point:[x,last.y],reason:cart?'小车在这里过不去：检查台阶、坡向或门洞高度。':'这里走不通：补上缺口，或把台阶变矮一点。'}}states=next;}let result=states.find(a=>a.y<=target[1]+.05);if(!result)return{ok:false,path:states[0].path,point:states[0].path.at(-1),reason:'已经走到这里了，还需要把平台搭高一点。'};return{ok:true,path:result.path}}
